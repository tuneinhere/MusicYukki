        ...
        if name[0:3] == "sta":
            ...
            def get_stats():
                ...
                for vidid, count in list_arranged.items():
                    ...
                    if vidid == "telegram":
                        msg += f"🔗[ᴛᴇʟᴇɢʀᴀᴍ ᴍᴇᴅɪᴀ](https://t.me/ArabxRobot) ** ᴩʟᴀʏᴇᴅ {count} ᴛɪᴍᴇs**\n\n"
                    else:
...
async def welcome(client, message: Message):
    ...
    if config.PRIVATE_BOT_MODE == str(True):
        if not await is_served_private_chat(message.chat.id):
            await message.reply_text(
                "**Ini Bot Music Free Culik Aja ke GC atau CH kalian Bot Musicnya Juga Ada Fitur Menarik \n\nDev : @Arabnihnge.**"
            )
...
async def alive(client, message: Message):
    await message.reply_photo(
        ...
        caption=f"""**Ini Bot Music Free Culik Aja ke GC atau CH kalian Bot Musicnya Juga Ada Fitur Menarik \n\nDev : @Arabnihnge.**""",
...
async def verify(client, message: Message):
    if await is_served_user(message.from_user.id):
        await message.reply_text(
            text="Verif Dulu Nyet takutnya lu bot",
        )
        return
    await add_served_user(message.from_user.id)
    await message.reply_photo(
        photo=f"https://telegra.ph/file/7f08acd78577f99f60ff5.png",
        caption=f"""nah udah pokoknya""",
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("Support Group", url=config.SUPPORT_GROUP)]]
        ),
    )
